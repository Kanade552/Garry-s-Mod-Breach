...
function OpenClassMenu()
	if IsValid(ClassMenu_Frame) then return end
	...
	ClassMenu_Frame = vgui.Create("DFrame")
	ClassMenu_Frame:SetTitle("")
	ClassMenu_Frame:SetSize(width, height)
	ClassMenu_Frame:Center()
	ClassMenu_Frame:SetDraggable(true)
	ClassMenu_Frame:SetDeleteOnClose(true)
	ClassMenu_Frame:SetDraggable(false)
	ClassMenu_Frame:ShowCloseButton(true)
	ClassMenu_Frame:MakePopup()
	ClassMenu_Frame.Paint = function(self, w, h)
		draw.RoundedBox(2, 0, 0, w, h, Color(0, 0, 0))
		draw.RoundedBox(2, 1, 1, w - 2, h - 2, Color(90, 90, 95))
	end
	
	local maininfo = vgui.Create("DLabel", ClassMenu_Frame)
	...
	local panel_right = vgui.Create("DPanel", ClassMenu_Frame)
	...
	if selectedclass.showmodel == nil then
		smodel = TableRandom(selectedclass.models)
	else
	...
	local panel_left = vgui.Create("DPanel", ClassMenu_Frame)
	...
	--	ClassMenu_Frame:Close()
	--end
	/*
	local button_escort = vgui.Create("DButton", ClassMenu_Frame)
...
print("Gamemode loaded core/client/ui/menu_class.lua")