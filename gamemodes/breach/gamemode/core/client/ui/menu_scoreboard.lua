...
function RanksEnabled()
	return cvars.Bool("br_scoreboardranks", false)
end
...
function role_GetPlayers(role)
	...
	for k,v in pairs(player.GetAll()) do
		if v:Alive() and v.GetNClass and v:GetNClass() == role then
...
function ShowScoreBoard()
		...
		if plist.skip == nil or plist.skip == false then
			...
			for roles_i, role in ipairs(ALLCLASSES[plist.name].roles) do
				for allpl_k, pl in pairs(player.GetAll()) do
					if pl.GetNClass != nil and pl:Alive() and pl:GetNClass() == role.name then
	...
	for k,pl1 in pairs(playerlist) do
		for k2,pl in pairs(pl1.list) do
			if pl:GTeam() != TEAM_SCP and pl:GTeam() != TEAM_SPECTATOR then
				...
				if istable(frole) and isstring(frole.disguised_sorting) and ALLCLASSES[frole.disguised_sorting] != nil then
	...
	SB_Frame:Center()
	SB_Frame:MakePopup()
	SB_Frame.Paint = function(self, w, h)
	...
	local mainpanel = vgui.Create("DPanel", SB_Frame)
	...
	sb_panel_version:SetText("version 02.10.2021 by Kanade")
		...
		if #tab.list > 0 then
			...
			for k,v in pairs(tab.list) do
				...
				scroll_panel.Paint = function(self, w, h)
					...
					local txt = language.class_unknown or "Unknown"
					...
					if istable(frole) and v:GTeam() == TEAM_CHAOS then
					...
					draw.RoundedBox(0, width + 150, 0, 150, h, Color(0,0,0,120))
					draw.Text({
						text = txt,
						pos = {width + 224, h / 2},
						font = "sb_names",
...
print("Gamemode loaded core/client/ui/menu_scoreboard.lua")